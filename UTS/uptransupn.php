<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['id_trans_upn'])) {
        //query SQL
        $id_trans_upn_update = $_GET['id_trans_upn'];
...
//melakukan pengecekan apakah ada form yang dipost
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id_trans_upn_update = $_POST['id_trans_upn'];
    ...
    SET id_kondektur = '$id_kondektur',id_bus = '$id_bus',id_driver = '$id_driver',jumlah_km = '$jumlah_km',tanggal = '$tanggal' 
    WHERE id_trans_upn = '$id_trans_upn_update'";
    ...
    <div>
      <div>
        ...
        <main role="main">
          ...
          <form action="uptransupn.php" method="POST">
          <?php while($data = mysqli_fetch_assoc($result)): ?>
            <div class="" hidden >
              <label>ID Trans UPN</label>
              <input type="text" class="" placeholder="id trans upn" name="id_trans_upn" required="required" value=<?php echo $data['id_trans_upn']?>>
            </div> 
                <?php
                ...
                <div class="form">
                  ...
                  <select class="input_text" name="id_bus" id="id_bus">
                    ...
                    <?php while($data_bus = mysqli_fetch_array($data_id_bus)): ?>
                      <option value="<?php echo $data_bus['id_bus']; ?>" <?= $data_bus['id_bus' ]== $data['id_bus'] ? "selected" : "" ?>><?php echo $data_bus['id_bus']; ?></option>
                    <?php endwhile;?>
                  </select>
                </div>
                ...
                <div class="form">
                  ...
                  <select class="input_text" name="id_driver" id="id_driver">
                    ...
                    <?php while($data_driver = mysqli_fetch_array($data_id_driver)): ?>
                      <option value="<?php echo $data_driver['id_driver']; ?>" <?= $data_driver['id_driver' ]== $data['id_driver'] ? "selected" : "" ?>><?php echo $data_driver['nama']; ?></option>
                    <?php endwhile;?>
                  </select>
                </div>
